...
<?php
	...
	$stmt = $con -> prepare('SELECT * FROM blog WHERE blog_id=?');
	$stmt -> bind_param('s',$article_id);
	$stmt -> execute(); 
	$stmt -> store_result(); 
	$stmt -> bind_result($id,$article_id,$title,$category_id,$preamble,$paragraph,$picture,$featured,$blog_date,$keywordss,$comments_allowed);
	$numrows = $stmt -> num_rows();
	if($numrows > 0){
		while ($stmt -> fetch()) {
						$blog_date = new DateTime($blog_date);
      		$date_formatted = $blog_date->format('F j, Y'); 
		}
	}
