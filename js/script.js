...
//image handling for background pic
document.getElementById('profile_background_input').addEventListener('change', function () {
    const file1 = this.files[0];
    const errorDisplay1 = document.getElementById('file_error1');
    const preview1 = document.getElementById('image_preview1');
    ...
    if (!file1.type.startsWith('image/')) {
        errorDisplay1.textContent = 'Please select a valid image file (jpg, png, gif, etc).';
    ...
    if (file1.size > maxSize1) {
        errorDisplay1.textContent = 'Image must be less than 2MB.';
        this.value = ''; // reset input
        return;
    }
...
//image handling for profile pic
document.getElementById('profile_pic_input').addEventListener('change', function () {
    const file = this.files[0];
    const errorDisplay = document.getElementById('file_error');
    ...
    if (!file.type.startsWith('image/')) {
        errorDisplay.textContent = 'Please select a valid image file (jpg, png, gif, etc).';
        this.value = ''; // reset input
        return;
    }
    ...
    if (file.size > maxSize) {
        errorDisplay.textContent = 'Image must be less than 2MB.';
        this.value = ''; // reset input
        return;
    }
...
//make incomplete profile warning disapper after 4 seconds
document.addEventListener('DOMContentLoaded', function() {
console.log('DOM is ready!');
const profileWarning = document.getElementById('profileWarning');
setTimeout(() => {
    profileWarning.classList.remove('show');
    profileWarning.style.display = 'none';
    profileWarning.className = '';
}, 4000);
});